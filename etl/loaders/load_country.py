...
def main():
    logger.info("=== START country loader ===")
    ...
    logger.info(f"Number of countries: {len(country_names)}")
    # country_names = ['United States of America', 'Sweden'] # для отладки чтобы не дергать апи
    all_data = []

    for country in country_names:
        logger.info(f"Requesting: {country}")
        country_iso2 = country_to_iso2.get(country)
        if country_iso2 in existing_iso2:
            logger.info(f"⏩ {country} (ISO2={country_iso2}) уже есть в staging.country, пропускаем")
            continue
        ...
        if response.status_code == 200:
            ...
            if data:
                ...
                logger.info(f"✅ Загружено в БД: {country}")
            else:
                logger.warning(f"Пустой ответ для {country}")
        else:
            logger.warning(f"❌ Ошибка для {country}: {response.status_code} - {response.text}")
    ...
    logger.info("=== END country loader ===")
